...
function setSync(t){const e=$('#dataMode');if(e)e.textContent=`Shared live · ${t}`}
...
async function loadStaticBaseline(){
  ...
  return {state:{framework,processes,registers,tasks,milestones},flowManifest:manifest,flowChunks:chunks,shared:false};
}
async function loadBaseline(){
  try{
    const r=await fetch('/api/state',{cache:'no-store'});
    if(r.ok){const x=await r.json();if(x?.state&&x?.flowManifest&&x?.flowChunks)return {state:x.state,flowManifest:x.flowManifest,flowChunks:x.flowChunks,shared:true,version:Number(x.version||0),fileVersions:x.fileVersions||{}}}
  }catch{}
  return loadStaticBaseline();
}
async function init(){
  const b=await loadBaseline();state=b.state;flowManifest=b.flowManifest;flowChunks=b.flowChunks;
  if(b.shared){
    sharedVersion=b.version||0;sharedFileVersions=b.fileVersions||{};localStorage.removeItem(LS);
  }else{
    try{const l=JSON.parse(localStorage.getItem(LS)||'null');if(l?.state&&l?.flowChunks){state=l.state;flowChunks=l.flowChunks}}catch{}
  }
  rebuildFlows();bindShell();render();setupPolling();setSync(b.shared?'connected':'offline baseline · changes stay on this browser');
}
function bindShell(){
  ...
  $('#refreshBtn').onclick=async()=>{if(dirtyFiles.size&&!confirm('Unsaved local edits exist. Replace them with the latest shared state?'))return;const b=await loadBaseline();state=b.state;flowManifest=b.flowManifest;flowChunks=b.flowChunks;if(b.shared){sharedVersion=b.version||0;sharedFileVersions=b.fileVersions||{}}dirtyFiles.clear();rebuildFlows();saveLocal();render();setSync(b.shared?'shared state refreshed':'offline baseline refreshed')};
