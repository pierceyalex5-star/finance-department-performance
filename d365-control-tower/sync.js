...
async function pushGithub(silent=false){
  ...
  try{
    for(const k of keys){
      ...
      const result=await ghPut(path,localValue,remote.sha,token,`D365 Control Tower · ${k} · ${new Date().toISOString()}`);
