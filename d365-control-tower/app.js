...
function setSync(t){const e=$('#dataMode');if(e)e.textContent=`GitHub shared · ${t}`}
...
function saveLocal(){const serialChunks={};for(const [p,fs] of Object.entries(flowChunks))serialChunks[p]=fs.map(({_file,...f})=>f);localStorage.setItem(LS,JSON.stringify({state,flowChunks:serialChunks,savedAt:new Date().toISOString()}))}
...
async function init(){
  const b=await loadBaseline();state=b.state;flowManifest=b.flowManifest;flowChunks=b.flowChunks;dirtyFiles.clear();rebuildFlows();bindShell();render();setupPolling();saveLocal();setSync('ready · auto-refresh on');
}
function bindShell(){
  ...
  $('#refreshBtn').onclick=async()=>{if(dirtyFiles.size&&!confirm('Unsaved local edits exist. Replace them with the latest deployed version?'))return;const b=await loadStaticBaseline();state=b.state;flowManifest=b.flowManifest;flowChunks=b.flowChunks;dirtyFiles.clear();rebuildFlows();captureSyncBase();saveLocal();render();setSync('deployed version refreshed')};
